...
static void swap_list_elem(t_file ** father, t_file * previous, t_file * a, t_file * b) {
	ft_printf("start_swap -> ");
	if (father) {
		*father = b;
		a->next = b->next;
		b->next = a;
	...
	for (t_file * i = *list; i; i = i->next) {
		...
		for (t_file * i = *list; i->next; i = i->next) {
			if (get_flag(ls->flags, t) && compare_time(i, i->next))
				i == *list ? swap_list_elem(list, previous, i, i->next) : swap_list_elem(NULL, previous, i, i->next);
